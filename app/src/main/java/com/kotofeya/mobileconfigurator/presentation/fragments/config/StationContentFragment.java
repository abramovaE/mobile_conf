    ...
    public void onClick(View v) {
        ...
        if(version != null && version.equals("ssh_conn")){
            ...
            if(modemConfigSend != null && !modemConfigSend.isEmpty()
                ...
                if(statTransiver.getModem().equalsIgnoreCase("megafon")
                        && modemConfigSend.equalsIgnoreCase("beeline")){
                    command.append(SshConnection.MODEM_CONFIG_MEGAF_BEELINE_COMMAND);
                }
                else if(statTransiver.getModem().equalsIgnoreCase("beeline")
                        && modemConfigSend.equalsIgnoreCase("megafon")){
                    command.append(SshConnection.MODEM_CONFIG_BEELINE_MEGAF_COMMAND);
                }
            }
            Logger.d(Logger.STATION_CONTENT_LOG, "send command: " + command.toString());
            SshConnection connection = new SshConnection(ip,
                    ((StationContentFragment) fragmentHandler.getCurrentFragment()));
